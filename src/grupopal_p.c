...
int main(int argc, char *argv[]) {
  ...
  fscanf(fd, "%d", &nvec);
  if (argc == 4)
    nvec = atoi(argv[3]); // 4. parametro: numero de vectores
  ...
  t_clust =
      (t17.tv_sec - t12.tv_sec) + (t17.tv_nsec - t12.tv_nsec) / (double)1e9;
  ...
  t_camp =
      (t21.tv_sec - t20.tv_sec) + (t21.tv_nsec - t20.tv_nsec) / (double)1e9;
  ...
  for (i = ind; i < ngrupos; i++)
    fprintf(fd, "%6d", listag[i].nvecg);
  fprintf(fd, "\n");
  ...
  for (i = 0; i < NCAM; i++)
    fprintf(fd,
            "Campo: %2d - mmax: %4.2f (grupo %2d) - mmin: %4.2f (grupo %2d)\n",
            i, info_cam[i].mmax, info_cam[i].gmax, info_cam[i].mmin,
            info_cam[i].gmin);
  ...
  for (i = 0; i < NCAM; i++)
    printf("Campo: %2d - max: %4.2f (grupo %2d) - min: %4.2f (grupo %2d)\n", i,
           info_cam[i].mmax, info_cam[i].gmax, info_cam[i].mmin,
           info_cam[i].gmin);
